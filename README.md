...
页面提供合规确认、新建抓取任务、新源站探索、书籍导出、EasyVoice JSON 导出和 EasyVoice 语音转换入口。
...
- `explore-site` 和 `propose-config` 只生成可审计报告与候选通用配置；不会自动启用配置、不会自动写 Python 专属适配器。
