...
| `GET` | `/api/tts/progress` | `conversions`：EasyVoice 分组转换摘要 |
...
`browser` 可为 `http` 或 `visible`，默认 `http`。`visible` 只用于操作者显式选择的有界面 Chrome 抓取，不会绕过登录、付费墙、验证码或 DRM。
...
轮询 EasyVoice 批量转换输出目录。服务端默认扫描 `/Users/admin/docker-data/easyVoice`，也可通过
`NOVEL_CRAWLER_TTS_PROGRESS_ROOT` 指定更窄的输出根目录。响应只返回目录 ID、分组名、计数、阶段和文件大小，不返回本机绝对路径。
  ...
  "conversions": [
    {
      ...
      "failed_groups": 0,
      "running": true,
      "current_group": "twbook-100786922-1700-1799",
      "current_phase": "m4b_transcoding",
      "groups": [
        {
          "group": "twbook-100786922-1700-1799",
          "status": "running",
          "phase": "m4b_transcoding",
          "chapter_mp3": 100,
          "chapter_srt": 100,
          "assembled": {"mp3": true, "srt": true, "m4b": true, "m4b_size": 239599660},
